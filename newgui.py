...
class ClusterutilizationData:
    def __init__(self,cluster_lbl,cluster_pb,cluster_devices,device_lbl,task_lbl) -> None:
        self.cluster_label=cluster_lbl
        self.cluster_pb=cluster_pb
        self.cluster_devices=cluster_devices
        self.device_lbl=device_lbl
        self.task_lbl=task_lbl
class DisplayLabel:
...
def createFrame(window,bgColor):
    fr=tk.Frame(window,bg=bgColor)
    return fr
...
#Defining the Frames
topFrame=tk.Frame(root,bg=__background_color,width=800,height=530)
# middleFrame=tk.Frame(root,bg='yellow',width=800,height=130,padx=40)
bottomFrame=tk.Frame(root,width=800,bg=__background_f_color,height=70,padx=10)
